...
void get_landauer_buttiker_current(const std::vector<double>& energy,
    const std::vector<double>& transmission_up, const std::vector<double>& transmission_down,
    double* current_up, double* current_down, const double voltage)
{
	...
	for (int r = 0; r < number_energy_points; r++) {
		*current_up -= delta_energy * transmission_up.at(r)
		    * (fermi_function(energy.at(r) + voltage)
		        - fermi_function(energy.at(r) - voltage));
...
void read_transmission_from_textfile(std::vector<double> &transmission_up, std::vector<double> &transmission_down, std::vector<double> &energy, int &number_energy_points){
	...
	fstream my_file;
	my_file.open("/home/declan/Downloads/transmission/0.TaFeMgOFeTa.TRC_0.90ev", ios::in);
    int n = 2;
