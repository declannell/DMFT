...
int main()
{
	...
	std::vector<Eigen::MatrixXcd> gf_local_up(parameters.steps,
	    Eigen::MatrixXcd::Zero(parameters.chain_length, parameters.chain_length));
	std::vector<Eigen::MatrixXcd> gf_local_down(parameters.steps,
	    Eigen::MatrixXcd::Zero(parameters.chain_length, parameters.chain_length));
	std::vector<Eigen::MatrixXcd> gf_local_lesser_up(parameters.steps,
	    Eigen::MatrixXcd::Zero(parameters.chain_length, parameters.chain_length));
	std::vector<Eigen::MatrixXcd> gf_local_lesser_up_FD(parameters.steps,
	    Eigen::MatrixXcd::Zero(parameters.chain_length, parameters.chain_length));
	std::vector<Eigen::MatrixXcd> gf_local_lesser_down(parameters.steps,
	    Eigen::MatrixXcd::Zero(parameters.chain_length, parameters.chain_length));
	std::vector<std::vector<dcomp>> self_energy_mb_up(parameters.chain_length,
	    std::vector<dcomp>(parameters.steps)),
	    self_energy_mb_lesser_up(parameters.chain_length, std::vector<dcomp>(parameters.steps, 0));
	std::vector<std::vector<dcomp>> self_energy_mb_down(parameters.chain_length,
	    std::vector<dcomp>(parameters.steps)),
	...
	for (int m = 0; m < parameters.NIV_points; m++) {
        ...
        */
		...
		get_local_gf_r_and_lesser(parameters, kx, ky, self_energy_mb_down,
		    self_energy_mb_lesser_down, leads, gf_local_down, gf_local_lesser_down, m);

		for (int r = 0; r < parameters.steps; r++) {
			for (int i = 0; i < parameters.chain_length; i++) {
				for (int j = 0; j < parameters.chain_length; j++) {
					gf_local_lesser_up_FD.at(r)(i, j) = -1.0
		...
		dmft(parameters, m, kx, ky, self_energy_mb_up, self_energy_mb_down,
		    self_energy_mb_lesser_up, self_energy_mb_lesser_down, gf_local_up, gf_local_down,
		    gf_local_lesser_up, gf_local_lesser_down, leads, spins_occup);
		...
		if (parameters.hubbard_interaction == 0) {
			get_transmission(parameters, kx, ky, self_energy_mb_up, leads, transmission_up,
			    transmission_down, m);
			...
			get_meir_wingreen_current(parameters, kx, ky, self_energy_mb_up,
			    self_energy_mb_lesser_up, leads, m, &current_up_MW.at(m));
		} else {
			get_meir_wingreen_current(parameters, kx, ky, self_energy_mb_up,
			    self_energy_mb_lesser_up, leads, m, &current_up_MW.at(m));
			get_meir_wingreen_current(parameters, kx, ky, self_energy_mb_down,
			    self_energy_mb_lesser_down, leads, m, &current_down.at(m));
		}
        ...
        */
		...
		for (int i = 0; i < parameters.chain_length; i++) {
			for (int r = 0; r < parameters.steps; r++) {
				myfile2 << parameters.energy.at(r).real() << "," << gf_local_up.at(r)(i, i).real()
		...
		myfile3.open(
		    "/home/declan/green_function_code/quantum_transport/textfiles/c++_tranmission.txt");
		// myfile << parameters.steps << std::endl;
		for (int r = 0; r < parameters.steps; r++) {
			myfile3 << parameters.energy.at(r).real() << "," << transmission_up.at(r).real() << ","
			        << transmission_up.at(r).imag() << "," << transmission_down.at(r).real()
			        << "\n";
		}
		myfile3.close();
		...
		for (int i = 0; i < parameters.chain_length; i++) {
			for (int r = 0; r < parameters.steps; r++) {
				myfile5 << parameters.energy.at(r).real() << ","
		...
		myfile_se_lesser.open(
		    "/home/declan/green_function_code/quantum_transport/textfiles/c++se_mb_lesser.txt");
		for (int i = 0; i < parameters.chain_length; i++) {
			for (int r = 0; r < parameters.steps; r++) {
				myfile_se_lesser << parameters.energy.at(r).real() << ","
		...
		myfile6.open(
		    "/home/declan/green_function_code/quantum_transport/textfiles/c++_se_mb_lesser.txt");
		for (int i = 0; i < parameters.chain_length; i++) {
			for (int r = 0; r < parameters.steps; r++) {
				myfile6 << parameters.energy.at(r).real() << ","
				        << self_energy_mb_lesser_up.at(i).at(r).real() << ","
				        << self_energy_mb_lesser_up.at(i).at(r).imag() << "\n";
			}
		}
	...
	// myfile << parameters.steps << std::endl;
	for (int m = 0; m < parameters.NIV_points; m++) {
		std::cout << "The spin up current is " << current_up.at(m) << "The spin down current is "
		          << current_down.at(m) << "\n";
		std::cout << "The meir wingreen current is " << current_up_MW.at(m) << "\n";
		std::cout << (dcomp)current_up.at(m).real() / current_up_MW.at(m).real() << "\n";
		std::cout << "\n";
		myfile4 << parameters.voltage_l[m] - parameters.voltage_r[m] << ","
		        << current_up.at(m).real() << "," << current_down.at(m).real() << "\n";
	}
	myfile4.close();
