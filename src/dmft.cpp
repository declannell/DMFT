...
void dmft(Parameters &parameters, int voltage_step, std::vector<std::vector<dcomp>> &self_energy_mb_up, 
    ...
    std::vector<Eigen::MatrixXcd> old_green_function(parameters.steps, Eigen::MatrixXcd::Zero(parameters.num_cor, parameters.num_cor));
    while (difference > 0.0001 && count < parameters.self_consistent_steps){
        ...
        for(int j = 0; j < parameters.num_cor; j++) {//we only do the dmft loop over the correlated metal.
            ...
            impurity_self_energy_lesser_down, &spins_occup.at(i), &spins_occup.at(i + parameters.num_cor));
...
//if(voltage_step == 0){
//    std::vector<Eigen::MatrixXcd> gf_local_lesser_up_FD(parameters.steps, Eigen::MatrixXcd::Zero(parameters.num_cor, parameters.num_cor));
//    for(int r = 0; r < parameters.steps; r++){
//        for(int i = 0; i < parameters.num_cor; i++){
//            for(int j = 0; j < parameters.num_cor; j++){
